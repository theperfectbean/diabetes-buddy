...
SOURCE_DIR = PROJECT_ROOT / "data" / "sources" / "androidaps-docs" / "docs" / "EN"
EXCLUDE_PATTERNS = [
    "README.md", "LICENSE*", ".github/*", "CONTRIBUTING.md", "mkdocs.yml",
...
def infer_doc_type(file_path):
    ...
    if any(k in path_str for k in ["setup", "install", "build", "getting-started"]):
        return "setup"
    elif any(k in path_str for k in ["algorithm", "oref", "autotune", "autosens", "smb", "dosing", "loop"]):
        return "algorithm"
    elif any(k in path_str for k in ["troubleshoot", "faq", "error", "problem", "issue"]):
...
def process_and_ingest(start_index, num_files):
    ...
    collection = client.get_or_create_collection(name="androidaps_docs", metadata={"hnsw:space": "cosine"})
    ...
    # Get all files - only .md and .rst files in /docs/EN/ subdirectories
    all_files = []
    for ext in ['*.md', '*.rst']:
        for file_path in SOURCE_DIR.rglob(ext):
            ...
            for pattern in EXCLUDE_PATTERNS:
                if pattern in str(rel_path) or any(pattern in str(rel_path) for pattern in EXCLUDE_PATTERNS):
                    skip = True
                    break
            if not skip:
    ...
    for file_idx, (file_path, rel_path) in enumerate(batch_files, start=start_index):
        try:
            ...
            for chunk_idx, chunk in enumerate(chunks):
                metadata = {
                    "source": "androidaps_docs",
                    "repo": "androidaps-docs",
                ...
                file_chunks.append({
                    'id': f"androidaps_{file_idx}_{chunk_idx}",
                    'document': chunk,
                    'metadata': metadata
                })
...
def main():
    parser = argparse.ArgumentParser(description="Ingest AndroidAPS documentation into ChromaDB in batches")
    ...
    print(f"Starting AndroidAPS batch ingestion: start-index={args.start_index}, num-files={args.num_files}")
    ...
    results = collection.query(query_texts=["How does AndroidAPS autotune work?"], n_results=3)
    ...
    print("=== AndroidAPS Batch Ingestion Report ===")
