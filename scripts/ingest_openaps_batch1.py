...
# Target user documentation, not API references
SOURCE_DIR = PROJECT_ROOT / "data" / "sources" / "openaps-docs" / "docs" / "docs"
# Exclude non-user-facing content and API documentation
EXCLUDE_PATTERNS = [
    "README.md",
    "LICENSE",
    ".github",
    ...
    "openaps/",       # Python API module docs
    "_static/",       # Static assets
    "_templates/",    # Template files
    "Images/",        # Image directories (no text content)
]
...
def process_and_ingest(start_index, num_files):
    ...
    collection = client.get_or_create_collection(name="openaps_docs", metadata={"hnsw:space": "cosine"})
...
def main():
    parser = argparse.ArgumentParser(description="Ingest OpenAPS documentation into ChromaDB in batches")
    ...
    print(f"Starting OpenAPS batch ingestion: start-index={args.start_index}, num-files={args.num_files}")
    ...
    results = collection.query(query_texts=["How does OpenAPS autosens feature work?"], n_results=3)
    ...
    print("=== OpenAPS Batch Ingestion Report ===")
